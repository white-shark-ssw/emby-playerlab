...
require(f'RELEASE_TAG="v{version}-build{build_number}-dev"' in build, "versioned release tag mismatch")
require(f'RELEASE_IPA="OS-player-v{version}-build{build_number}-${{GITHUB_SHA::7}}-unsigned.ipa"' in build, "versioned release IPA mismatch")
