...
api = Path('Sources/Networking/EmbyLibraryHubAPI.swift')
text = api.read_text()
start = text.index('    func searchLandingRecommendations(')
end = text.index('    func movieRecommendations', start)
block = text[start:end]
old_sig = '    func searchLandingRecommendations(limit: Int = 9, includeItemTypes: [String] = []) async throws -> [LibraryItem] {'
new_sig = '    func searchLandingRecommendations(limit: Int = 9, includeItemTypes: [String] = [], excludeItemIds: [String] = []) async throws -> [LibraryItem] {'
if old_sig not in block:
    raise SystemExit('searchLandingRecommendations signature not found')
block = block.replace(old_sig, new_sig, 1)
needle = '        if !includeItemTypes.isEmpty { query.append(URLQueryItem(name: "IncludeItemTypes", value: includeItemTypes.joined(separator: ","))) }\n'
if needle not in block:
    raise SystemExit('searchLandingRecommendations IncludeItemTypes line not found')
block = block.replace(needle, needle + '        if !excludeItemIds.isEmpty { query.append(URLQueryItem(name: "ExcludeItemIds", value: excludeItemIds.joined(separator: ","))) }\n', 1)
api.write_text(text[:start] + block + text[end:])

preloader = Path('Sources/UI/EmbySearchRecommendationPreloader.swift')
text = preloader.read_text()
needle = '    static let preloadLimit = 9\n'
if needle not in text or 'static let loadMoreLimit = 6' in text:
    raise SystemExit('preloader policy anchor invalid')
text = text.replace(needle, needle + '    static let loadMoreLimit = 6\n', 1)
marker = '    private func beginStartupWarm(for stored: EmbySession, sessionStore: SessionStore) {'
if marker not in text:
    raise SystemExit('preloader startup marker not found')
method = '''    func moreRecommendations(client: EmbyAPIClient, excluding itemIDs: [String]) async throws -> [LibraryItem] {
...
preloader.write_text(text)

experience = Path('Sources/UI/EmbySearchExperienceV3.swift')
text = experience.read_text()
needle = '    private var recommendationGeneration = 0\n'
if needle not in text or 'private var isLoadingMoreRecommendations = false' in text:
    raise SystemExit('recommendation generation anchor invalid')
text = text.replace(needle, needle + '    private var isLoadingMoreRecommendations = false\n', 1)
needle = '            isLoadingRecommendations = false\n        } else {\n'
if needle not in text:
    raise SystemExit('toggleRecommendations anchor not found')
text = text.replace(needle, '            isLoadingRecommendations = false\n            isLoadingMoreRecommendations = false\n        } else {\n', 1)
needle = '            recommendationItems = items\n            hasMoreRecommendations = false\n'
if needle not in text:
    raise SystemExit('loadRecommendations result anchor not found')
text = text.replace(needle, '            recommendationItems = items\n            hasMoreRecommendations = items.count >= V3SearchRecommendationPolicy.preloadLimit\n', 1)
marker = '    func recommendationPosterImage(for itemID: String) -> UIImage? { recommendationPosterImages[itemID] }'
if marker not in text:
    raise SystemExit('poster image marker not found')
method = '''    func loadMoreRecommendations(client: EmbyAPIClient) async {
...
'''
text = text.replace(marker, method + marker, 1)
section_start = text.index('    private var recommendationsSection: some View {')
section_end = text.index('    private var searchResults: some View {', section_start)
section = text[section_start:section_end]
needle = '''                        V3SearchRecommendationPosterCard(item: item, client: currentClient, pinnedImage: model.recommendationPosterImage(for: item.id)) { image in
                            model.pinRecommendationPosterImage(image, for: item.id)
                        }
'''
if needle not in section:
    raise SystemExit('recommendation card anchor not found')
replacement = needle + '''                        .onAppear {
...
experience.write_text(text[:section_start] + section + text[section_end:])
...
for i, line in enumerate(lines):
    if line.startswith('- Status:'):
        lines[i] = '- Status: Active — Build253 recommendation data path target-device accepted; Build254 incremental recommendation candidate implementation/CI in progress'
        break
text = '\n'.join(lines).rstrip() + '\n'
marker = '## Build253 target-device acceptance → Build254 incremental recommendations — 2026-08-30'
if marker not in text:
