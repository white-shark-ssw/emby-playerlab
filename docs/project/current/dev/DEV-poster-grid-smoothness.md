...
Build278 / OnePlayer **0.15.11 (278)** exact source `6ff8113d9c45dfae6d745afa98b4a04a3956cf33`, Draft PR #281, is target-device tested with `OnePlayer-App-1788165599.log`. The diagnostic purpose is complete; do not merge this measurement-only PR.

The log supplies direct timing proof. For each accepted +60 pagination state from **120→660 Library items**, `PagePersistentCache` completes the full Library snapshot only **1–8 ms** before the paired severe display gap. Snapshot total and gap pairs are: **38.31→49.96**, **43.98→58.29**, **52.63→66.70**, **58.46→66.70**, **62.42→75.06**, **66.62→75.06**, **70.02→79.17**, **74.72→86.56**, **78.77→91.67**, **94.66→108.33 ms**. Pearson correlation is approximately **0.991** and the median `display_gap - snapshot_total` residual is approximately **12.24 ms**.

At **480 items**, the snapshot costs **70.02 ms** and ends at `08:34:33.102`; the **79.17 ms** display gap is logged at `08:34:33.103` with `insert_events=0`, and `insert-begin` occurs only at that same later timestamp. This independently demonstrates that the persistence work alone can consume the missed display interval before collection insertion begins. Across the scaling series, object construction grows **7.77→21.42 ms**, JSON serialization is the dominant component at **27.31→62.30 ms**, and atomic write is normally **1.97–3.55 ms** with one **10.01 ms** 660-item sample. Bytes grow from **739,044→3,026,705**.

No-append control sessions remain healthy even at high accumulated item counts: fixed 540 items is **119.67 Hz** with max **17.74 ms** and zero >=25 ms gaps; fixed 600 is **118.82 Hz** with max **20.59 ms** and zero >=25 ms; fixed 660 sessions are **119.69 / 120.35 Hz** with max **17.65 / 11.02 ms** and zero >=25 ms. Reverse events in the long sessions are boundary bounce, not an interior offset correction.

This proves synchronous full-Library presentation persistence is a **major direct contributor to pagination-adjacent large twitch/long frames**. It does not prove it is the universal historical poster-smoothness root: Build229 already showed moving persistence off-main did not eliminate every broader Library hitch family. Therefore the next A/B must be narrow and preserve the accepted Build213 cache contract: accepted fresh state still writes through atomically and cached-first relaunch remains unchanged, but snapshot serialization/write must not occupy the scrolling `@MainActor` frame path. Do not combine this with container, pagination, image, scroll-physics, Search, Home or P0 changes.

**Evidence:** Build278 Code/exact scope/checker/CI/IPA ✅ / target-device tested ✅ / synchronous full-snapshot persistence → pagination severe-tail causal contribution ✅ / universal 3×3 root ❌ / stable ❌.
