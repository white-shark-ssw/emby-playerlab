...
Build259 / 0.14.92 exact source `39168e560d7e626557de8ebde6a88a5d38b3478b` is now target-device tested. `OnePlayer-App-1788087127.log` confirms the 80→device-max request moves fixed-item Library/Search display p50/p95 from Build258 ~16.67 ms to normally 8.34 ms; the user reports obvious jitter is now difficult to see. The remaining EX-vs-OnePlayer difference is subtler: EX feels like one smooth/fine curve while OnePlayer still feels slightly coarse/wave-like. Fixed-item offset KVO often shows p50 ~8.4 ms but p95 ~16.8 ms; this is only a diagnostic lead because KVO can coalesce and does not justify guessing scroll physics. Search recommendation append tails remain a separate secondary layer and Build256 semantics stay protected. PR #267 is closed without merge; Build259 is proven useful but not final/stable.

Build260 / 0.14.93 exact source `b9a5de5255650f04e312e117f47453122de56adc`, superseded Draft PR #268, directly extends Build259 with diagnostic-only per-display-frame sampling of real `UIScrollView.contentOffset.y` during native deceleration. It preserves the same single cadence display link and high-refresh request, and records zero-move frames, zero→catch-up pairs, reversals, movement-step distributions and consecutive step-ratio distributions. It does not change `decelerationRate`, ScrollView physics, Grid layout, pagination, images/caches, Search, Home carousel, Player or Transport. Xcode 16.4 run/job `33307963917 / 99247767453` succeeded; artifact `9731113592`, digest `sha256:293055cff1d8524a19ac4e21b39bb2b90afc7451fb1311c4760efc53d08739f8`; IPA SHA `1434d2b31c7ced4f344b2e946c5311d2c287774cfe72bbcbd527a24a1ccbffe8`; source ZIP SHA `875ea8a22e4aca0924c58e70faf250a891a8b97ce980a300bc6bcf1ee16998db`; MinOS 15.0. Evidence: Code/CI/IPA verified; target-device diagnostic completed; no fix/stable claim.

Build261 / 0.14.94 exact source `e552bebd072a915e6cb10d591d704a5a3c342406`, Draft PR #270, directly extends Build260. Home reuses the existing real vertical `UIScrollView` observer and requests 80→device-max only during drag/deceleration; target-device logs now show Home display p50/p95 8.34 ms in all 12 captured sessions and the user reports it is noticeably better, although max 50.31/36.24/76.94 ms tails remain. Library adds same-frame long-gap attribution: 49 sessions / 9,573 display samples contain 356 `>=12.5 ms`, 94 `>=25 ms`, 57 `>=33.3 ms` gaps. One fixed 600→600 fast-scroll session has 73 long gaps with 67 overlapping cell churn, proving cell lifecycle work is a direct contributor; fixed-count sessions still reach 112.97 ms and 162 total long gaps are untracked, proving a second source. Build261 is Code/CI/IPA/real-device tested but not stable. Do not treat 120 Hz request, pagination optimization, physics tuning or a wholesale Grid replacement as a complete solution without isolating the severe untracked tail.

Build263 / 0.14.96 exact source `bff02ea8e76217b1fe07c298d8b9058b2db1fd08`, Draft PR #271, is now target-device diagnostic tested with `OnePlayer-App-1788098393.log`. Across 26 Library sessions / 5,557 display samples it records 40 gaps `>=25 ms` and 27 `>=33.3 ms`. In 19 fixed-item sessions, cell churn overlaps 26/28 severe25 and all 16/16 severe33; image publication overlaps 23/28 severe25 and 15/16 severe33, leaving only 2 fixed severe25 and zero fixed severe33 untracked. One fixed 775→775 fast pass concentrates up to 18 cell appears + 18 disappears and 27 image publications in a long-gap interval, while another fixed 775 pass performs hundreds of cell/image events but has zero severe25. This establishes burst concentration during high-speed poster-cell realization/image publication as the current lead, not pagination or guessed scroll physics. Build263 is diagnostic tested, not a fix and not stable.

Build266 / 0.14.99 exact source `957e88dcdc408e537d63b083d0f30e4b1157b1dc`, former Draft PR #273 now closed without merge, is target-device tested with `OnePlayer-App-1788105516.log`. Home is clearly smoother and can reach 120 FPS; its three sessions all have display p50/p95/p99 8.34 ms, with only sparse 23–33 ms tails. Shared 3×3 remains unresolved: 15 Library sessions / 9,576 display samples contain 52 gaps >=25 ms and 33 >=33.3 ms, max 130.12 ms. In 11 fixed-item sessions, 20/21 severe25 overlap cell churn, 18/21 image publication, and all 12/12 severe33 overlap both. Build266 therefore rejects unused loader `isLoading` publication as a sufficient 3×3 fix; not stable.

Build267 / 0.15.0 exact source `dbe1b7c13dde68e52039cb7ae22fc5177fdd886f`, Draft PR #275, directly extends Build266 and changes only the grid diagnostic session from value to reference semantics plus identity/checker/changelog. The existing diagnostic `MotionSession` holds several growing arrays and was repeatedly copied/mutated/reassigned from high-frequency KVO/display callbacks, creating a concrete Swift Array copy-on-write self-overhead risk. All diagnostic fields, thresholds and the 80→device-max request remain; product Grid/image/pagination/Search/scroll physics are unchanged. Xcode 16.4 run/job `33321883421 / 99285172527` and cleanup `99285673130` succeeded; artifact `9735159629`, digest `sha256:bbabd6c84752a8437a9c23c02b6bdecc60915b9d6403d96d203745b8b9589661`; IPA SHA `54c2b8851d54aee126ff12f5c4f1a54f6fa62ca0293517fc137fc25b94ef3d3c`; source ZIP SHA `0ae52e34cc4aafb383ec7a7c4ca8adf1846df51c038b5a8f44fd7bc6ea9dad3d`; MinOS 15.0. Evidence: Code/CI/IPA verified; target-device pending; not stable. Home Build265 / PR #274 is kept separate for carousel image-tail testing.
...
## Active correction: Home carousel presented performance — Build269 / Build270 / Build271
...
Build274 changed exactly three product files from Build271; protected Hero/Interaction/State blobs are unchanged. Xcode 16.4 run/job `33333236724 / 99315483085` passed; artifact `9738285110`, digest `sha256:0fb0e5f9a07c6eb16eab00cdf516283991b0d5e6d61597ea368497ccb4f320f7`; IPA SHA `2fc79d5d09aa8e0c2f6384b4a50e933cf79f885c4b8d9fd05932fc1a3cc6295a`; source ZIP SHA `ed85b8c7a1d28de8af26ba7124386dfa987b3e83d8c4d38b61e8b5b61c4d5598`; independent re-download confirms IPA integrity, `0.15.7 (274)`, bundle `com.embyplayerlab.app`, MinOS 15.0 and MinOS audit OK. Evidence is **Code written ✅ / CI passed ✅ / IPA produced+independently verified ✅ / target-device TREE120 pending / stable ❌**.
