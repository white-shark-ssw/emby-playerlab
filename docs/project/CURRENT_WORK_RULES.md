...
- **Baseline**：`docs/strict-session-routing`，基于 `main` 双工作槽机制。
...
- **Completed**：已确认“无法判断时必须询问用户，禁止默认选择”是硬规则；已将三条短口令、Active 状态不可作为路由依据、ambiguity-stop 写入路由器、`AGENTS.md` 与文档策略。
- **Validation state**：Rule documented on branch
- **Pending**：检查最终 diff、创建并合并 PR、将本槽恢复为 `Idle`。
- **Next exact action**：检查 `docs/strict-session-routing` 相对 `main` 的改动范围，确认只涉及规则文件。
- **Rejected / do-not-repeat**：根据 Active 状态、历史会话、最近更新、任务紧急程度或模型猜测自动选槽；在语义不明确时擅自将任一槽设为 `Active`。
