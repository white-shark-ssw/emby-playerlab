...
| Home carousel interaction | **Active Build187 diagnostic candidate; Build185 real-device rejected** | Build185 restored the required full-page slide but target-device EX recordings still showed 10/12/16 px first visible movement vs EX 1/1/2 px and coarser ongoing increments. Build186 added passive cadence timing but its generic log was not exportable through the existing playback-log UI. Build187 / 0.14.20 changes only that log channel to playback; dedicated Release CI passed and IPA was produced. Real-device recording + exported `HomeCarouselDragTiming` evidence is pending. |
...
| Detail episode selection navigation | **Active Build188 candidate / real-device pending** | Detail horizontal episode cards now select-only with the existing blue selected outline and compact selected-episode summary; main Play/Resume remains the playback action. Full picker no longer dismisses before playback, so closing player should reveal the same picker/scroll position. Build188 / 0.14.21 passed dedicated Release CI and produced an IPA; do not mark stable until target-device validation. Build182 scroll/cache, Build176 player session replacement and Build178 canonical order remain unchanged. |
...
| Other product modules | Active parallel work | Build184 / 0.14.17 is the accepted overall runtime baseline on `main`. Build187 / 0.14.20 carousel diagnostic and Build188 / 0.14.21 detail episode-selection candidate are independent real-device-pending lines; neither replaces Build184 until accepted. |
