...
| Home carousel interaction | **Active; Build185 real-device rejected, Build186 diagnostic direction** | Build180 fixed reversal continuity but remained coarse. Build183 crossfade felt somewhat finer but was rejected because it changed the established foreground slide interaction. Build185 restored full-page foreground slide and removed the 1.08 axis gate, yet new target-device recordings still show first visible movement around 10/12/16 px versus EX around 1/1/2 px and coarser ongoing increments. ProMotion opt-in is present, so the next step is to measure DragGesture delivery/cadence before further rendering changes. |
...
| Other product modules | Active parallel work | Build184 / 0.14.17 is the accepted overall runtime baseline on `main`. The next carousel candidate must inherit this accepted baseline rather than continue from the older isolated carousel branch. |
