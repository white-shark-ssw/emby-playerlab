...
| Player episode selection / auto-next | **Active Build195 performance follow-up; Build194 grouping correctness confirmed** | Build194 TrollStore-friendly rewrap installed successfully on the target device and the supplied 980-episode non-standard Series now displays the full episode set, confirming the SeasonId-first grouping fix. New real-device issue: opening the in-player picker with all 980 episodes blocks for several seconds. Source inspection shows `ScrollView(.horizontal) + HStack + ForEach(displayedEpisodes)` eagerly constructs every complex card/image view. Build195 is reserved to replace only that episode-row container with `LazyHStack`, keeping the full canonical 980-item data, SeasonId grouping, current picker UI, source-owned session replacement and trusted-natural-end auto-next unchanged. |
