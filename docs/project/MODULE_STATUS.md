...
| Home carousel interaction | **Active Build186 diagnostic candidate; Build185 real-device rejected** | Build180 fixed reversal continuity but remained coarse. Build183 crossfade felt somewhat finer but was rejected because it changed the established foreground slide interaction. Build185 restored full-page foreground slide and removed the 1.08 axis gate, yet target-device recordings still show first visible movement around 10/12/16 px versus EX around 1/1/2 px and coarser ongoing increments. Build186 inherits accepted Build184 `main` plus Build185 carousel semantics and adds passive drag cadence logging only; dedicated Release CI passed and IPA was produced, real-device diagnosis pending. |
...
| Other product modules | Active parallel work | Build184 / 0.14.17 is the accepted overall runtime baseline on `main`. Build186 / 0.14.19 is the current independent carousel diagnostic candidate and must not replace Build184 until target-device acceptance. |
