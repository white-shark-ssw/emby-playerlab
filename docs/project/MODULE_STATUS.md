...
| Player episode selection / auto-next | **Active Build195 large-list performance candidate; Build194 grouping correctness confirmed** | Build194 TrollStore-friendly rewrap installed successfully and the supplied 980-episode non-standard Series displays the full episode set, confirming SeasonId-first grouping. Build194 exposed several-second picker-open blocking from an eager 980-card `HStack`; Build195 changes only that row to `LazyHStack` and dedicated Xcode 16.4 Release CI passed. The first distributed Build195 phone copy failed before runtime with TrollStore helper `168`, and the user observed that received IPA was only about 5 MB versus the verified CI IPA's 18,258,195 bytes, so this is recorded as a distribution truncation rather than a LazyHStack runtime rejection. A 46.77 MiB fully-stored rewrap from the same CI-built app is pending install/performance validation. Full canonical data, SeasonId grouping, picker UI, source-owned session replacement and trusted-natural-end auto-next remain unchanged. |
...
| Other product modules | Active parallel work | Build191 / 0.14.24 is the accepted overall runtime baseline on `main`. Build192 / 0.14.25 Add/Edit Emby has real-device feedback and is superseded by Build196 / 0.14.29, whose cached-first/password follow-up has CI/IPA complete but awaits target-device validation. Build193 remains the rejected/investigation carousel line; Build194 proved player SeasonId grouping correctness; Build195 is the CI/IPA-complete lazy-row candidate whose first distributed copy was truncated before runtime testing and now awaits install/performance validation with the full-size rewrap. |
