...
| Poster 3-column / poster-heavy scroll smoothness | **Active — Build202 CI/IPA verified; real-device pending** | `DEV-poster-grid-smoothness` owns OnePlayer 0.14.35 / Build202. User recording proves an existing stop-one-recorded-frame → catch-up hitch and also proves the problem is broader than `EmbyPosterGrid` because Home uses `LazyHStack + V3PosterCard`. Build202 keeps existing lazy containers and removes source-proven common-path overhead: grid Environment wrappers move to the ancestor; ordinary posters skip unused callback-state and invisible loading-state publications; unchanged nil image state is not republished; 118 pt Home posters request actual device-pixel width instead of fixed 440 px; actor result posters use the no-loading-indicator path. Exact-source run/job `32993726508` / `98257448257` passed; artifact `9615751921` was independently downloaded and its digest matched GitHub; IPA/source ZIP integrity, `0.14.35 (202)` identity and MinOS 15.0 were verified. No Player/MPV/PiP/Transport/Cache/Session or carousel-owner file changed. Target-device improvement is still unproven, so this is not stable. |
