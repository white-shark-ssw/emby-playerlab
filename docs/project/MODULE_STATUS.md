...
| Home carousel interaction | **Active Build189 native-touch candidate; Build187 diagnostic confirmed** | Build187 target-device logs showed first useful SwiftUI horizontal samples already about 4.33/8.00/15.67/11.00pt with maxFPS=120 and Low Power Mode off, so threshold tuning is retired. Build189 / 0.14.22 changes only manual drag sampling to UIKit raw/coalesced touches while preserving full-page slide and original predicted release commit; dedicated Release CI passed and IPA was produced. Real-device validation is pending. |
...
| Detail episode selection navigation | **Active Build190 candidate; Build188 real-device follow-up required** | Build188 established select-only horizontal episode cards, compact selected-episode summary and non-dismissing full picker playback, but target-device feedback found no default visible selection on series entry and quick range buttons cleared selection/title. Build190 / 0.14.23 now selects explicit initial episode → resumable episode → canonical first episode, and quick range buttons select that range's first episode instead of clearing state. Dedicated Release CI passed and IPA was produced; real-device validation is pending. Build182 scroll/cache, Build176 player session replacement and Build178 canonical order remain unchanged. |
...
| Other product modules | Active parallel work | Build184 / 0.14.17 is the accepted overall runtime baseline on `main`. Build189 / 0.14.22 carousel native-touch and Build190 / 0.14.23 detail episode-selection are independent real-device-pending lines; neither replaces Build184 until accepted. |
