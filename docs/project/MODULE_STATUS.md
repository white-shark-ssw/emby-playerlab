...
| Poster 3-column grid smoothness | **Active — Stage 1 code written / draft PR #259** | `DEV-poster-grid-smoothness` is isolated on `perf/poster-grid-smoothness`. Stage 1 keeps the existing `LazyVGrid`, grid geometry, pagination trigger, native navigation and image pipeline, and only hoists the two identical per-cell grid Environment injections to the grid ancestor. Exact task diff is the shared grid file plus a narrow checker; no Build198 carousel file or frozen playback/transport/cache path is touched. CI/IPA/target-device A/B are pending, so this is not yet a validated performance improvement. Global `EmbyCachedRemoteImage` optimization is deferred because it is a shared dependency with the active Build198 Home Hero. |
